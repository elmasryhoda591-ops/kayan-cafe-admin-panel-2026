...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    const unsubMenu = onSnapshot(collection(db, 'menuItems'), (snapshot) => {
      ...
      if (snapshot.empty && isAdmin) {
        seedKanMenu().catch(console.error);
      }
    });
  ...
  const handleSeedData = async () => {
    setIsSubmitting(true);
    setMessage({ text: '', type: '' });
    if (window.confirm("تحذير: هذا سيقوم بحذف جميع الأصناف الموجودة وإعادة ضبط المنيو الافتراضي. هل أنت متأكد؟")) {
      const success = await seedKanMenu();
      if (success) {
        setMessage({ text: 'تمت إعادة ضبط المنيو بنجاح!', type: 'success' });
      } else {
        setMessage({ text: 'حدث خطأ أثناء إعادة ضبط المنيو', type: 'error' });
      }
    }
    setIsSubmitting(false);
  };
        ...
        <div className="p-6 md:p-8">
          ...
          {activeTab === 'menu' && (
            <div className="space-y-8">
              <div className="pb-6 border-b border-analog-border">
                <button
                  type="button"
                  onClick={handleSeedData}
                  disabled={isSubmitting}
                  className="bg-red-500 hover:bg-red-600 text-white px-6 py-3 rounded-lg font-mono tracking-wider uppercase text-sm disabled:opacity-70 flex items-center justify-center gap-2 w-full md:w-auto"
                >
                  <ShieldAlert size={18} />
                  إعادة ضبط المنيو للوضع الافتراضي (نسخة كان كافيه)
                </button>
                <p className="text-analog-muted text-xs mt-2 font-sans">
                  سيتم مسح جميع البيانات الحالية للمنيو وتعويضها بالبيانات الافتراضية.
                </p>
              </div>
              <form onSubmit={handleAddMenuItem} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input required name="title" type="text" className="w-full px-4 py-3 border border-analog-border rounded-lg focus:ring-1 focus:ring-analog-coral focus:border-analog-coral outline-none bg-analog-900 text-white font-sans" placeholder="مثال: قهوة تركي" />
                  </div>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block font-mono text-xs tracking-widest text-analog-muted uppercase mb-2">السعر *</label>
                    <input required name="price" type="text" className="w-full px-4 py-3 border border-analog-border rounded-lg focus:ring-1 focus:ring-analog-coral focus:border-analog-coral outline-none bg-analog-900 text-white font-sans" placeholder="مثال: 45" />
                  </div>
                  <div>
                    <label className="block font-mono text-xs tracking-widest text-analog-muted uppercase mb-2">تصنيف فرعي (اختياري)</label>
                    <input name="subCategory" type="text" className="w-full px-4 py-3 border border-analog-border rounded-lg focus:ring-1 focus:ring-analog-coral focus:border-analog-coral outline-none bg-analog-900 text-white font-sans" placeholder="مثال: سنجل / دبل" />
                  </div>
                </div>

                <div>
                  <label className="block font-mono text-xs tracking-widest text-analog-muted uppercase mb-2">الوصف (اختياري)</label>
                  <textarea name="description" rows={2} className="w-full px-4 py-3 border border-analog-border rounded-lg focus:ring-1 focus:ring-analog-coral focus:border-analog-coral outline-none bg-analog-900 text-white font-sans" placeholder="وصف مكونات الصنف..."></textarea>
                </div>

                <div>
                  <label className="block font-mono text-xs tracking-widest text-analog-muted uppercase mb-2">صورة الصنف (اختياري)</label>
                  <input 
                    accept="image/*" 
                    onChange={(e) => handleImageUpload(e, setMenuImage)} 
                    type="file" 
                    className="w-full px-4 py-3 border border-analog-border rounded-lg focus:ring-1 focus:ring-analog-coral focus:border-analog-coral outline-none bg-analog-900 text-white font-sans file:mr-4 file:py-2 file:px-4 file:rounded-full file:bg-analog-800 file:text-analog-coral" 
                  />
                  {menuImage && (
                    <div className="mt-4 relative inline-block">
                ...
                <button disabled={isSubmitting} type="submit" className="w-full flex items-center justify-center gap-2 bg-analog-coral text-white py-4 px-4 rounded-xl hover:bg-analog-coral-hover transition-colors font-mono tracking-wider uppercase disabled:opacity-70 mt-8">
                  {isSubmitting ? 'جاري الإضافة...' : <><PlusCircle size={20} /> إضافة الصنف للمنيو</>}
                </button>
              </form>

              {menuItems.filter(item => !item.id.startsWith('kan_')).length > 0 && (
                <div className="space-y-4">
                  <h3 className="font-serif italic text-xl text-white">الأصناف المضافة بواسطة الإدارة</h3>
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {menuItems.filter(item => !item.id.startsWith('kan_')).map((item, idx) => (
                      <div key={item.id} className="relative bg-analog-900 border border-analog-border rounded-xl p-4 flex flex-col gap-2">
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-bold text-white leading-tight">{item.title}</h4>
                            <span className="text-xs text-analog-muted">{categoryNames[item.category] || item.category}</span>
                          </div>
                          <span className="font-mono text-analog-coral shrink-0 text-sm">{item.price} ج.م</span>
                        </div>
                        {item.imageUrl && (
                          <img src={item.imageUrl} alt={item.title} className="w-full h-32 object-cover rounded-lg mt-2 border border-analog-border" />
                        )}
                        <div className="mt-2 flex justify-between items-center bg-analog-950/50 p-2 rounded-lg border border-analog-border/50">
                           <div className="flex gap-2">
                             <button
                               disabled={idx === 0}
                               onClick={() => handleSwapOrder('menuItems', menuItems.filter(i => !i.id.startsWith('kan_')), idx, 'up')}
                               className="p-1.5 bg-analog-800 hover:bg-analog-700 transition-colors text-white rounded-lg disabled:opacity-30"
                               title="نقل لأعلى"
                             >
                               <ArrowUp size={14} />
                             </button>
                             <button
                               disabled={idx === menuItems.filter(i => !i.id.startsWith('kan_')).length - 1}
                               onClick={() => handleSwapOrder('menuItems', menuItems.filter(i => !i.id.startsWith('kan_')), idx, 'down')}
                               className="p-1.5 bg-analog-800 hover:bg-analog-700 transition-colors text-white rounded-lg disabled:opacity-30"
                               title="نقل لأسفل"
                             >
                               <ArrowDown size={14} />
                           ...
                           <button
                             onClick={() => handleDeleteImage('menuItems', item.id)}
                             className="p-1.5 bg-red-900/30 hover:bg-red-900 text-red-500 rounded-lg transition-colors border border-red-900/50 flex items-center justify-center"
                             title="حذف الصنف"
                           >
                             <Trash2 size={14} />
                           </button>
            ...
            <div className="space-y-8">
              <form onSubmit={handleUpdateCovers} className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-analog-800 p-6 rounded-xl border border-analog-border">
                    <label className="block font-serif italic text-lg text-white mb-2">غلاف قائمة المنيو</label>
